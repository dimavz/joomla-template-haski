...
class jtt_tpl_links extends JoomlaTuneTemplate
{
	function render() 
	{
		...
		if ($readmoreLink != '' || $commentsLink != '') {
			...
			<div class="jcomments-links">
				...
				<?php echo $commentsLink; ?> <?php echo $hitsCount; ?>
			</div>
	...
	function getReadmoreLink() 
	{
		...
		return '<a class="'.$css.'" href="'.$link.'" title="'.htmlspecialchars($title).'">'.'<button class="seemore btn btn-info">'.$text.'</button></a>';
	}
	...
	function getCommentsLink()
	{
		...
		switch($style) {
			case -1:
				return $count > 0 ? '<span class="'.$css.'"><button class="btn btn-info">'.$text.'</button></span>' : '';
				break;
			default:
				return '<a class="'.$css.'" href="'.$link.'" title="'.htmlspecialchars($text).'"><button class="btn btn-warning">'.$text.'</button></a>';
				break;
		}
	}
}